...
- **Data = self-host ChordMini's MIT `python_backend`** on the Donnacha VPS (45.77.233.102, Docker). Endpoints Alight calls: `POST /api/recognize-chords` (chords + timestamps), `POST /api/detect-beats` (beats + BPM), `GET /api/lrclib-lyrics` (synced lyrics via LRCLIB). This is the hard, already-built MIR layer.
