...
3. Does the SOCKS endpoint egress through Perth?
   ...
   ```bash
   ssh root@45.77.233.102 'curl --socks5-hostname 172.17.0.1:1080 https://api.ipify.org'
   # Should return your Perth residential IP, not 45.77.233.102.
   ```

4. Cookies still valid?

   ```bash
   ssh root@45.77.233.102 'wc -l /opt/chordmini-recon/cookies/youtube.txt'
...
5. yt-dlp + deno still in the running container?
   ...
   ```bash
   ssh root@45.77.233.102 'docker exec chordmini-alight bash -c "/opt/venv/bin/yt-dlp --version; /usr/local/bin/deno --version"'
   ```
