// PlayApp.jsx — root of the Play view. Steps through a song.
// Spacebar = next chord. Left/Right arrows = prev/next.

const { useState, useEffect, useCallback, useRef } = React;

function PlayApp() {
  const song = window.PCH_SONG;
  const [idx, setIdx] = useState(0);
  const [playing, setPlaying] = useState(false);
  const [voicing, setVoicing] = useState("simple");      // simple | full
  const [layout, setLayout]   = useState("sectioned");   // sectioned | flat
  ...
  const next = song.steps[idx + 1] || song.steps[0]; // loop preview

  // Theme on root element.
  useEffect(() => {
    document.documentElement.dataset.theme = theme;
  }, [theme]);

  const goPrev = useCallback(() => setIdx(i => (i - 1 + song.steps.length) % song.steps.length), [song.steps.length]);
  const goNext = useCallback(() => setIdx(i => (i + 1) % song.steps.length), [song.steps.length]);
  ...
  useEffect(() => {
    if (!playing) return;
    const ms = (60 / bpm) * 1000 * 4; // one chord per bar at 4 beats
    const t = setInterval(() => setIdx(i => (i + 1) % song.steps.length), ms);
    return () => clearInterval(t);
  }, [playing, bpm, song.steps.length]);

  // Tap-tempo: record taps, compute median interval.
  const taps = useRef([]);
  function onTap() {
    ...
    <div className="play-page">
      <header className="topbar">
        ...
        <div className="topbar-right">
          <button className="icon-btn" title="Songbook" aria-label="Songbook">
          ...
          <button className="icon-btn" title="Print this song" aria-label="Print this song">
          ...
          <button className="icon-btn" title={`Switch to ${theme === "light" ? "dark" : "light"} theme`}
      ...
      <main className="play-stage">
        <div className="chord-row">
          ...
          <div className="chord-label-side">
            <div className="capo">{song.capo}</div>
        ...
        <div className="keyboards">
          <div className="hand-block">
            ...
            <Keyboard hand="left"
              startNote="C2" endNote="E3"
              nowNotes={cur.left} nextNotes={next.left}
              size="md" showFingering={fingering} />
          </div>
          <div className="hand-block">
            ...
            <Keyboard hand="right"
              startNote="F3" endNote="B4"
              nowNotes={cur.right} nextNotes={next.right}
              size="md" showFingering={fingering} />
          </div>
        </div>
      ...
      <footer className="play-footer">
        <div className="footer-controls">
          ...
          <Tempo bpm={bpm} onTap={onTap} metronome={metronome} onMetronomeChange={setMetronome} />
          <Transpose semitones={semitones} onChange={setSemitones} />
        </div>
      </footer>
    </div>
  );
